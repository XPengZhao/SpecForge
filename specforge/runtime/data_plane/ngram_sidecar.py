...
class NgramSidecar:
    def __init__(self, root, manifest):
        ...
        if not meta:
            raise ValueError(
                "Ngram Markov requires a DeepSpec raw ngram_embedding sidecar"
            )
        expected = dict(
            version=1,
            dtype="bfloat16",
            index_record_format=RECORD.format,
            index_record_size=RECORD.size,
        ...
        for key, value in expected.items():
            if meta.get(key) != value:
                raise ValueError(
                    f"Ngram sidecar {key}: expected {value!r}, got {meta.get(key)!r}"
                )
        ...
        for shard in meta["shards"]:
            ...
            if (
                ...
                or path.stat().st_size != shard["num_bytes"]
            ):
